      ...
      case Regex.test(args):
        ...
        switch (Input.action) {
          ...
          case "add":
            ...
            db.prepare("insert into PersistentRoles (roleId) values (?)")
              .run(role.id);
              
            // Tell em everything's good
            await msg.channel.createMessage({
              content: "I will now give returning members the **" + role.name + "** role if they had it before they left.",
              messageReferenceID: msg.id,
              allowedMentions: {
                repliedUser: true
              }
          ...
          case "remove":
            ...
            if (db.prepare("select * from PersistentRoles where roleId = (?)").get(RoleId)) {
              
              // Delete it
              db.prepare("delete from PersistentRoles where roleId = (?)").run(RoleId);
              
            };
            
            // Done!
            await msg.channel.createMessage({
              content: "I will no longer give returning members that role!",
              messageReferenceID: msg.id,
              allowedMentions: {
                repliedUser: true
              }
          ...
          case "list":
            ...
            const Roles = db.prepare("select * from PersistentRoles").all();
            var descRoles = "";
            ...
            await msg.channel.createMessage({
              content: "Here are the roles I'm giving the returning members now:",
              embed: {
